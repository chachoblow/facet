...
The step-4 test pass landed a follow-up batch that pins the contracts step 5 will extend or rely on: `findBlocks` on a setext heading (empty marker range), on a GFM task list item (still a plain `listItem` with no `checked` field — step 5's `checked` extension will be obviously additive), on a tilde-fenced code block, and on an indented (4-space) code block. The webview also now has its own vitest setup (`apps/vscode/test/`); `buildDecorations` is exported from `block-marks.ts` and covered by three cursor-in-block assertions guarding the off-by-one we already hit once (cursor on the heading line, cursor at end-of-line, cursor on the next line).
